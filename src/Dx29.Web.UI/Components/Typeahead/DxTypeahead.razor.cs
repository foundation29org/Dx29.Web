...
{
    public partial class DxTypeahead<TItem, TValue> : ComponentBase, IDisposable
    {
        ...
        [Parameter] public EventCallback<string> OnSearchText { get; set; }
        ...
        {
            Initialize(Suggestions.Length > 0);
        }

        private void Initialize(bool clearText = true)
        {
            if (clearText)
            {
                SearchText = "";
        ...
        private async Task HandleKeyup(KeyboardEventArgs args)
        {
            ...
            else if (args.Key == "Enter" && Suggestions.Count() == 0)
            {
        ...
        {
            const string resultClass = "dx-typeahead__active-item";
            ...
            if (Equals(value, Value) || (Values?.Contains(value) ?? false))
            {
                ...
                {
                    return "dx-typeahead__selected-item-highlighted";
                }

                return "dx-typeahead__selected-item";
            }
        ...
        {
            static public ValueTask<object> Focus(IJSRuntime jsRuntime, ElementReference element)
            {
                return jsRuntime.InvokeAsync<object>("blazoredTypeahead.setFocus", element);
            }

            static public ValueTask<object> AddKeyDownEventListener(IJSRuntime jsRuntime, ElementReference element)
            {
