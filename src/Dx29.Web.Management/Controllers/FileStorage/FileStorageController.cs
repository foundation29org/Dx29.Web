...
namespace Dx29.Web.Controllers
{
    ...
    public class FileStorageController : ControllerBase
    {
        ...
        public FileStorageController(FileStorageService fileStorageService, UserServices userServices)
        {
        ...
        [RequestSizeLimit(SIZE_LIMIT)]
        [HttpGet("{caseId}/{resourceId}/{name}")]
        public async Task<IActionResult> DownloadAsync(string caseId, string resourceId, string name)
        {
            if (name.Length > 128) return BadRequest("Name too large.");
            ...
            {
                string userId = UserServices.GetUserId();
                var stream = await FileStorageService.DownloadAsync(userId, caseId, resourceId, name);
        ...
        [HttpDelete("{caseId}/{resourceId}/{name}")]
        public async Task<IActionResult> DeleteAsync(string caseId, string resourceId, string name)
        {
            if (name.Length > 128) return BadRequest("Name too large.");
